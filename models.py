...
#     def __init__(self, in_channels, hidden_channels, out_channels, shared, args):
...
#
#         if self.shared:
#             self.coef = Parameter(torch.Tensor(args.K + 1))
#         else:
#             self.register_parameter('coef', None)
#
#         self.convs.append(BernConv(in_channels, hidden_channels, args.K, shared=self.shared))
#         for _ in range(args.L - 2):
#             self.convs.append(BernConv(hidden_channels, hidden_channels, args.K, shared=self.shared))
#         self.convs.append(BernConv(hidden_channels, out_channels, args.K, shared=self.shared))
#
#         self.reset_parameters()
#
#     def reset_parameters(self):
#         if self.shared:
#             self.coef.data.fill_(1)
#         for conv in self.convs:
#             conv.reset_parameters()
#
#     def forward(self, x, edge_index):
#         for i, conv in enumerate(self.convs[:-1]):
#             x = conv(x, edge_index, self.coef)
#             x = F.relu(x)
#             x = F.dropout(x, p=self.dropout, training=self.training)
#         x = self.convs[-1](x, edge_index, self.coef)
#         return x
#
#
#
...
#             self.register_parameter('coef', None)
#
#         self.convs.append(ChebIIConv(in_channels, hidden_channels, args.K, shared=self.shared))
#         for _ in range(args.L - 2):
#             self.convs.append(ChebIIConv(hidden_channels, hidden_channels, args.K, shared=self.shared))
#         self.convs.append(ChebIIConv(hidden_channels, out_channels, args.K, shared=self.shared))
#
#         self.reset_parameters()
#
#     def reset_parameters(self):
...
#         self.convs = nn.ModuleList()
#         self.shared = shared
#         self.points = torch.linspace(0, 2, steps=args.K + 1)
#
#         if self.shared:
#             self.coef = Parameter(torch.Tensor(args.K + 1))
#         else:
#             self.register_parameter('coef', None)
#
#         self.convs.append(NewtonConv(in_channels, hidden_channels, args.K, self.points, shared=self.shared))
#         for _ in range(args.L - 2):
#             self.convs.append(NewtonConv(hidden_channels, hidden_channels, args.K, self.points, shared=self.shared))
#         self.convs.append(NewtonConv(hidden_channels, out_channels, args.K, self.points, shared=self.shared))
#
#         self.reset_parameters()
#
#     def reset_parameters(self):
