...
def train(model, optimizer, train_mask, ratio):
    ...
    class_loss = criterion(z[train_mask], data.y[train_mask])
...
def search_hyper_params(trial: optuna.Trial):
    ...
    dprate = trial.suggest_categorical("dprate", [0.0, 0.1, 0.3, 0.5, 0.7])
    weight_decay = trial.suggest_categorical("weight_decay", [0.0, 5e-4])
    K = trial.suggest_categorical("K", [5, 8, 11])
    hidden = trial.suggest_categorical("hidden", [64, 256])
    gamma = trial.suggest_categorical('gamma', [0.0, 1.0, 3.0, 5.0, 10.0, 20.0])
    gamma2 = trial.suggest_categorical('gamma2', [0.0, 1.0, 3.0, 5.0, 10.0, 20.0])
    gamma3 = trial.suggest_categorical('gamma3', [0.0, 1.0, 3.0, 5.0, 10.0, 20.0])
